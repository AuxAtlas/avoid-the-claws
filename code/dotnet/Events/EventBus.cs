...
public partial class EventBus : Node, IService
{
    ...
    public void Subscribe<T>(Action<T, KableConnection?> handler) where T : GameEvent
    {
    ...
    public void Unsubscribe<T>(Action<T, KableConnection?> handler) where T : GameEvent
    {
        if (!_handlers.ContainsKey(HashHelper.FastHash<T>()))
            return;
    ...
    public void Publish<T>(T @event, KableConnection? source = null) where T : GameEvent
    {
        if (_handlers.ContainsKey(HashHelper.FastHash<T>()))
            _handlers[HashHelper.FastHash<T>()].ForEach(x => ((Action<T, KableConnection?>)x)(@event, source));
    }
}